...
# 1. 페이지 설정
st.set_page_config(page_title="프리미엄 팀 빌딩 시스템 Pro", layout="wide")

# 2. CSS 스타일 (z-index 수정으로 팝업창 가림 현상 해결)
st.markdown("""
<style>
    /* 팝업창을 감싸는 컨테이너 */
    .tooltip { 
    ...
    /* 실제 정보가 나타나는 팝업창 (중요: z-index를 999,999로 설정하여 최상단 배치) */
    .tooltip .tooltiptext {
        visibility: hidden; 
        width: 300px; 
        ...
        font-size: 11px; 
        line-height: 1.5; 
        box-shadow: 0px 8px 30px rgba(0,0,0,0.8); /* 그림자를 더 진하게 하여 가독성 확보 */
        word-break: keep-all; 
        pointer-events: none; 
    ...
    th, td { border: 1px solid #ddd; overflow: hidden; height: 40px; position: relative; } /* td에도 position 설정 */
    .time-col { background:#f8f9fa; font-size: 11px; color: #666; text-align: center; width: 100px; font-weight: bold; }
</style>
""", unsafe_allow_html=True)

# 3. 유틸리티 함수 (기존 로직 100% 동일 유지)
def get_total_min(t):
    h = t.hour
    if h < 5: h += 24
...
# 5. 사이드바 제어판 (모든 기능 및 엑셀 저장 방식 유지)
with st.sidebar:
    st.header("⚙ 시스템 통합 관리")
